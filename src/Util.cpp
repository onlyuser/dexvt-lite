...
{
    if(up_direction) {
        glm::mat4 orient_xform = GLM_ORIENT(ORIENT_YAW(orient), ORIENT_PITCH(orient), ORIENT_ROLL(orient));
        *up_direction = glm::vec3(orient_xform * glm::vec4(VEC_UP, 1));
        return glm::vec3(orient_xform * glm::vec4(VEC_FORWARD, 1));
    }
    glm::mat4 orient_xform_sans_roll = GLM_ORIENT_SANS_ROLL(ORIENT_YAW(orient), ORIENT_PITCH(orient));
