...
* For the IK constraints part, I disagree with [this reply](https://stackoverflow.com/questions/21373012/best-inverse-kinematics-algorithm-with-constraints-on-joint-angles) on stackoverflow. Rather than allowing ball joints with constraints, the solver should limit itself to hinge joints with constraints. So instead of rotating axis by axis and later rejecting solutions where one axis' rotation violates constraints, the solver shouldn't attempt illegal rotations at all. I propose a solution that only attempts [legal rotation objectives](https://github.com/onlyuser/dexvt-lite/blob/master/src/TransformObject.cpp#L366) and enforces constraints [in cartesian space](https://github.com/onlyuser/dexvt-lite/blob/master/src/TransformObject.cpp#L216) (as opposed to euler space, which is suceptible to gimbal issues when a euler coordinate crosses the zenith).
